...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-BkQjDIEr.mjs";
...
const EventActionPanel = ({
  ...
  const emailTemplateName = ["q4enok6b2tgm3td9l7voh4hk", "q4enok6b2tgm3td9l7voh4hk", "qoq891lngymb8o99jbtwat1e"];
    ...
    try {
      for (let i = 0; i < emailTemplateName.length; i++) {
        ...
        if (res.data.data && res.data.data.template) {
          ...
          const title = formValues.title ?? "[Please insert title here]";
          const abstract = formValues.abstract ?? "[Please insert abstract here]";
          let eventDate = typeof formValues.eventDate === "string" ? formValues.eventDate : "[Please insert date here]";
          let eventStartTime = typeof formValues.eventStartTime === "string" ? formValues.eventStartTime : "[Please insert start time here]";
          let eventEndTime = typeof formValues.eventEndTime === "string" ? formValues.eventEndTime : "[Please insert end time here]";
          const speaker = formValues.speaker ?? "[Please insert speaker here]";
          let eventTypeFormatted = "event";
          if (typeof formValues.eventType === "string") {
            if (!formValues.eventType.includes("other") && formValues.eventType.trim() !== "") {
              eventTypeFormatted = formValues.eventType.trim();
            }
          }
          let physicalLocation = formValues.physicalLocation ?? "";
          let teamsLink = formValues.teamsLink ?? "";
          let locationFormatted = "[Please enter location and microsoft teams link here]";
          if (typeof physicalLocation === "string" && typeof teamsLink === "string") {
            physicalLocation = physicalLocation.trim();
            teamsLink = teamsLink.trim();
            if (physicalLocation && teamsLink) {
              locationFormatted = `This event will be hybrid - both in person in ${physicalLocation}, and online on MS Teams at ${teamsLink}`;
            } else if (physicalLocation) {
              locationFormatted = `This event will be in person in ${physicalLocation}`;
            } else {
              locationFormatted = `This event will be online on MS Teams at ${teamsLink}`;
            }
          }
...
//# sourceMappingURL=EditViewPage-BJZv4Vje.mjs.map
