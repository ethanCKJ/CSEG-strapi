...
const index = require("./index-BGHQjnYk.js");
  ...
  endpoints: (builder) => ({
    ...
    getAllDocuments: builder.query({
      query: ({ model, params }) => ({
        ...
        method: "GET",
        config: {
          params: qs$1.stringify(params, { encode: true })
        }
      }),
...
const buildValidParams = (query2) => {
  if (!query2) return query2;
  const { plugins: _, ...validQueryParams } = {
    ...query2,
    ...Object.values(query2?.plugins ?? {}).reduce(
...
const useDocumentLayout = (model) => {
  ...
  const [{ query: query2 }] = admin.useQueryParams();
  const { toggleNotification } = admin.useNotification();
  const { _unstableFormatAPIError: formatAPIError } = admin.useAPIErrorHandler();
...
const useDoc = () => {
  ...
  const [{ query: query2 }] = admin.useQueryParams();
  const params = React__namespace.useMemo(() => buildValidParams(query2), [query2]);
  if (!collectionType) {
...
const useDiscardAction = (activeTab, collectionType, model, document2, documentId) => {
  ...
  const [{ query: query2 }] = admin.useQueryParams();
  const params = React__namespace.default.useMemo(() => buildValidParams(query2), [query2]);
  if (!schema?.options?.draftAndPublish) {
    return null;
  }
  return {
  ...
  const handleEmailTemplates = async () => {
    ...
    try {
      for (let i = 0; i < emailTemplateTitles.length; i++) {
        title = emailTemplateTitles[i];
        query = qs.stringify({});
        const res = await get(`/content-manager/collection-types/api::text-email-template.text-email-template/${documentId}`);
      }
    } catch (e) {
...
//# sourceMappingURL=EditViewPage-DMKdlQGu.js.map
