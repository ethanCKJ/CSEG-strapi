...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-BOMI--RV.mjs";
  ...
  const handleEmailTemplates = async () => {
    ...
    try {
      for (let i = 0; i < emailTemplateName.length; i++) {
        ...
        if (Array.isArray(res.data?.results) && res.data.data.results.length > 0 && res.data.data.results[0].template) {
...
//# sourceMappingURL=EditViewPage-DWeQrjZP.mjs.map
