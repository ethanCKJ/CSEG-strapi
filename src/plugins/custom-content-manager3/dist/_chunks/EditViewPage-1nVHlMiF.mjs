...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-Cbg50CKE.mjs";
  ...
  const handleEmailTemplates = async () => {
    ...
    try {
      for (let i = 0; i < emailTemplateName.length; i++) {
        ...
        console.log("res", res, ".data.data.results", res.data?.data?.results);
        console.log("resArray", Array.isArray(res.data?.data?.results));
        console.log("resLength", res.data.data.results.length);
        console.log("T", res.data.data.results[0].template);
        if (Array.isArray(res.data?.data?.results) && res.data.data.results.length > 0 && res.data.data.results[0].template) {
          const textTemplate = res.data.data.results[0].template;
...
//# sourceMappingURL=EditViewPage-1nVHlMiF.mjs.map
