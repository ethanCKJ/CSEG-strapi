...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-abK_0E7t.mjs";
...
const useDocumentLayout = (model) => {
  ...
  const [{ query: query2 }] = useQueryParams();
  const { toggleNotification } = useNotification();
  const { _unstableFormatAPIError: formatAPIError } = useAPIErrorHandler();
...
const useDoc = () => {
  ...
  const [{ query: query2 }] = useQueryParams();
  const params = React.useMemo(() => buildValidParams(query2), [query2]);
  if (!collectionType) {
...
const useDiscardAction = (activeTab, collectionType, model, document2, documentId) => {
  ...
  const [{ query: query2 }] = useQueryParams();
  const params = React__default.useMemo(() => buildValidParams(query2), [query2]);
  if (!schema?.options?.draftAndPublish) {
    return null;
  }
  return {
...
//# sourceMappingURL=EditViewPage-DBb9zx1i.mjs.map
