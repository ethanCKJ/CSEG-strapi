...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-C40y3hfr.mjs";
  ...
  const handleEmailTemplates = async () => {
    ...
    try {
      for (let i = 0; i < emailTemplateName.length; i++) {
        ...
        onChange(`emailSubject${i + 1}`, `${eventTypeFormatted} - ${eventDate && formatSubjectDate(eventDateRaw)} - ${title}`);
...
//# sourceMappingURL=EditViewPage-Bl7EPrDT.mjs.map
