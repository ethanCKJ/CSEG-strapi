...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-wnlwBkzr.mjs";
  ...
  const handleEmailTemplates = async () => {
    ...
    try {
      for (let i = 0; i < emailTemplateName.length; i++) {
        ...
        console.log("res", res);
        if (Array.isArray(res.data?.data?.results) && res.data.data.template) {
...
//# sourceMappingURL=EditViewPage-CmCyvrbO.mjs.map
