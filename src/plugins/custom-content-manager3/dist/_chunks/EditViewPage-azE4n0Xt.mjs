...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-BKIHESkh.mjs";
...
const EventActionPanel = ({
  ...
  const emailTemplateName = ["xhf1je3muvhzgka92d5ooelw", "y4fjycg0agtpz4leq2qr74y6", "qoq891lngymb8o99jbtwat1e"];
    ...
    try {
      for (let i = 0; i < emailTemplateName.length; i++) {
        ...
        const query = qs.stringify({
          filters: {
            templateName: {
              $eq: "event-announcement-email"
            }
          }
        });
        const res = await get(`/content-manager/collection-types/api::text-email-template.text-email-template?${query}`);
        console.log("res", res);
        if (Array.isArray(res.data?.data?.results) && res.data.data.results.length > 1 && res.data.data.results[0].template) {
...
//# sourceMappingURL=EditViewPage-azE4n0Xt.mjs.map
