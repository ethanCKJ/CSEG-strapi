...
import { S as SINGLE_TYPES, g as getTranslation, C as COLLECTION_TYPES, a as CLONE_PATH, L as LIST_PATH, M as MEMBER_APPLICATION_MODEL, A as APPLICATION_STATUS, E as EVENT_MODEL } from "./index-BUPI0OPD.mjs";
...
const useDocument = (args, opts) => {
  ...
  const getInitialFormValues = React.useCallback(
    (isCreatingDocument = false) => {
      ...
      const form2 = document2?.id ? document2 : createDefaultForm(schema, components);
      return transformDocument(schema, components)(form2);
    },
...
const RepeatableComponent = ({
  ...
  const handleClick = () => {
    if (value.length < max) {
      ...
      const form2 = createDefaultForm(schema, components);
      const data = transformDocument(schema, components)(form2);
...
const DynamicZone = ({
  ...
  const handleAddComponent = (uid, position) => {
    ...
    const data = transformations(form2);
...
const UIDInput = React.forwardRef(
  ({ hint, label, labelAction, name: name2, required, attribute = {}, ...props }, ref) => {
    ...
    const allFormValues = useForm("InputUID", (form2) => form2.values);
...
//# sourceMappingURL=EditViewPage-C4bVrY5U.mjs.map
