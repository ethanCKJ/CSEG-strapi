...
export default ({ strapi }: { strapi: Core.Strapi }) => ({
  canConfigureContentType({
    ...
    const action = contentTypesUtils.isSingleType(contentType)
      ? 'plugin::custom-content-manager3.single-types.configure-view'
      : 'plugin::custom-content-manager3.collection-types.configure-view';
  ...
  async registerPermissions() {
    ...
    const actions = [
      {
        ...
        displayName: 'Create',
        uid: 'explorer.create',
        pluginName: 'custom-content-manager3',
    ...
    await strapi.admin.services.actionProvider.registerMany(actions);
  },
});
