...
import { ContentManagerPlugin } from './custom-content-manager2';
...
export default {
  register(app: any) {
    ...
    app.addMenuLink({
      ...
      icon: Feather,
      intlLabel: {
        id: `custom-content-manager2.plugin.name`,
    ...
    app.widgets.register((widgets: WidgetArgs[]) => {
      const lastEditedWidget: WidgetArgs = {
        ...
        permissions: [{ action: 'plugin::custom-content-manager2.explorer.read' }],
      };
