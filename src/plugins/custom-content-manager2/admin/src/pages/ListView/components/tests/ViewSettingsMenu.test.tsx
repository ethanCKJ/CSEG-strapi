...
const render = (props?: Partial<ViewSettingsMenuProps>) =>
  renderRTL(<ViewSettingsMenu setHeaders={jest.fn()} resetHeaders={jest.fn()} {...props} />, {
    renderOptions: {
      wrapper({ children }) {
        return (
          <>
            <Routes>
              <Route path="/custom-content-manager2/:collectionType/:slug" element={children} />
            </Routes>
            <LocationDisplay />
          </>
        );
    ...
    initialEntries: ['/custom-content-manager2/collection-types/api::address.address'],
  });
...
describe('ViewSettingsMenu', () => {
  ...
  it('should navigate to the configuration page when I click on the configure the view button', async () => {
    ...
    await screen.findByText(
      '/custom-content-manager2/collection-types/api::address.address/configurations/list'
    );
  });
});
