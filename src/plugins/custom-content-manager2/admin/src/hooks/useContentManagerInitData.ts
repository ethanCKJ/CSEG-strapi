...
const useContentManagerInitData = (): AppState => {
  ...
  const state = useTypedSelector((state) => state['custom-content-manager2'].app);
  ...
  return links
    ...
    .map((link) => {
      const collectionTypesPermissions = [
        { action: 'plugin::custom-content-manager2.explorer.create', subject: link.uid },
        { action: 'plugin::custom-content-manager2.explorer.read', subject: link.uid },
      ];
      ...
      return {
        ...
        to: `/custom-content-manager2/${link.kind === 'collectionType' ? COLLECTION_TYPES : SINGLE_TYPES}/${
          link.uid
        }`,
        uid: link.uid,
