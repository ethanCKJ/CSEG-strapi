...
const RecentDocumentsTable = ({
  ...
  const getEditViewLink = (document: RecentDocument): string => {
    ...
    return `/custom-content-manager2/${kindPath}/${document.contentTypeUid}${isSingleType ? '' : '/' + document.documentId}${queryParams}`;
  };
            ...
            <Td>
              <CellTypography variant="omega" textColor="neutral600" $maxWidth={dynamicMaxWidth}>
                {document.kind === 'singleType'
                  ? formatMessage({
                      id: 'custom-content-manager2.widget.last-edited.single-type',
            ...
            <Td onClick={(e) => e.stopPropagation()}>
              <Box display="inline-block">
                <IconButton
                  ...
                  label={formatMessage({
                    id: 'custom-content-manager2.actions.edit.label',
                    defaultMessage: 'Edit',
                  })}
                  variant="ghost"
                >
...
const LastEditedWidget = ({ columnWidth = 6 }: { columnWidth?: number }) => {
  ...
  if (data.length === 0) {
    return (
      <Widget.NoData>
        {formatMessage({
          id: 'custom-content-manager2.widget.last-edited.no-data',
...
const LastPublishedWidget = ({ columnWidth = 6 }: { columnWidth?: number }) => {
  ...
  if (data.length === 0) {
    return (
      <Widget.NoData>
        {formatMessage({
          id: 'custom-content-manager2.widget.last-published.no-data',
      ...
      >
        ...
        <text x={RADIUS} y={RADIUS} textAnchor="middle" fontSize="2.4rem" fontWeight="bold">
          ...
          >
            {formatMessage(
              {
                id: 'custom-content-manager2.widget.chart-entries.count.label',
        ...
        >
          ...
          >
            <Typography textColor="neutral0">
              {formatMessage(
                {
                  id: 'custom-content-manager2.widget.chart-entries.tooltip',
    ...
    <Flex minHeight="100%">
      <DonutChartSVG
        data={[
          {
            label: formatMessage({
              id: 'custom-content-manager2.containers.List.draft',
          ...
          {
            label: formatMessage({
              id: 'custom-content-manager2.containers.List.modified',
          ...
          {
            label: formatMessage({
              id: 'custom-content-manager2.containers.List.published',
